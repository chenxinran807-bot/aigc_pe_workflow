...
- 已生成第二条具名局部图 Case 的专项审查页 `work/named-detail-extra-review.html`：同时展示用户图、搭配图、生成图、人工手部标签和 5 条模型额外发现；三张图片已验证可加载，支持逐条判定与 JSON 导出。
...
- 对第二条新增的 5 条额外发现进行三图人工复核；在复核完成前记录为 `unverified_extra_finding`，不直接计为正式误报。
...
等待用户在 `work/named-detail-extra-review.html` 完成 5 条额外发现复核并导出 JSON；读取结论后汇总原图、contact-sheet 和具名局部图三种输入的能力边界。不进入验证集，也不启用模型协作。
