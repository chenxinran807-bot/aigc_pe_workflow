...
const result = await cdp.send("Runtime.evaluate", {
  ...
  expression: `(() => {
    ...
    return {
      controls: Array.from(document.querySelectorAll("input,textarea,[contenteditable=true]")).filter(visible).map(describe),
      fieldControls: Array.from(document.querySelectorAll(".field-item--E1HRs6Ya6Ok1N9Sx")).filter(visible).map(field => ({
        fieldText: field.innerText,
        rect: rect(field),
        controls: Array.from(field.querySelectorAll("input,textarea,[contenteditable=true]")).filter(visible).map(describe),
        html: field.outerHTML.slice(0, 2000)
      }))
    };
  })()`,
});
